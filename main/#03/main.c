...
void Dioda_Rand_5 (void * pvparameters);
...
void vTaskPrioritySet( xTaskHandle xTask, unsigned portBASE_TYPE uxNewPriority ) PRIVILEGED_FUNCTION;
 ...
 xTaskHandle xHandle;
	...
	*/
	...
	xTaskCreate( Dioda_Rand_5, ( signed char * ) "Dioda_5", 100, NULL, 2, &xHandle_5 );
...
void Przycisk_1( void *pvparameters )
{
	...
	uint8_t wasPressed = 0;
		...
		for (;;){
			...
			{
				//Zwiêkszenie priorytetu zadania Przycisk_1
			...
			//OpóŸnienie sprawdzaj¹ce co 30[ms], czy przycisk zosta³ wciniêty
			vTaskDelay(xDelay);
		}
}
...
void Dioda_2( void *pvparameters )
{
	//Sekwencja diód wyœwietlana po wciœniêciu przycisku
	...
	while(1){
		...
		//Przywrócenie zawieszonych zadañ
...
void Dioda_3( void *pvparameters )
{
	//Zadanie mrygaj¹ce diod¹ co sekundê
	...
	while(1)
	{
		//OpóŸnienie startu zadania o 5[s]
		 vTaskDelayUntil( &xLastWakeTime, xFrequency );
...
void Dioda_Rand_5( void *pvparameters )
{
	//Zadanie losowo mrygaj¹ce diodami okreœlon¹ sekwencjê
	...
	while(1)
	{
		//OpóŸnienie startu zadania o 10[s]
		vTaskDelayUntil( &xLastWakeTime, xFrequency );

		//Usuniêcie zadania, gdy licznik osi¹gnie okreœlon¹ wartoœæ
		if(counter==6)
		{
...
void Akcelerometr_6( void *pvparameters )
{
	//Zadanie mrygaj¹ce diodami sekwencjê, gdy oœ z osi¹gnie okreœlon¹ wartoœæ
